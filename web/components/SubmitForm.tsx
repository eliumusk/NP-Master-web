...
export function SubmitForm({ isLoggedIn }: { isLoggedIn: boolean }) {
  ...
  async function submitUpload(e: React.FormEvent) {
    ...
    if (!r1.ok) {
      ...
      setPhase("error"); setError(j.error ?? `创建任务失败 (${r1.status})`); return;
    }
  ...
  async function submitAccession(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (!accession.trim()) { setError("请输入 NCBI accession"); return; }
    setPhase("fetching");
    ...
    if (!r.ok) {
      ...
      setPhase("error"); setError(j.error ?? `拉取失败 (${r.status})`); return;
    }
  ...
  return (
    <div className="space-y-6">
      {/* Source tabs */}
      <div className="flex gap-1 rounded-md border border-slate-200 p-0.5 text-sm dark:border-slate-800">
        {([
          { v: "upload", label: "上传文件" },
          ...
          <button
            key={t.v}
            type="button"
            onClick={() => { setSource(t.v); setError(null); }}
        ...
        {source === "upload" ? (
          <div
            ...
            className={`relative cursor-pointer rounded-xl border-2 border-dashed p-10 text-center transition-colors ${
              dragOver ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-950/30"
          ...
          >
            ...
            <div className="mt-1 text-xs text-slate-500">
              ...
              {!isLoggedIn && <>（<span className="text-slate-700 dark:text-slate-300">登录可放宽到 50 MB</span>）</>}
            </div>
            {sniffOk !== null && (
          ...
          <div className="space-y-3">
            <label className="block">
              <span className="text-sm font-medium">NCBI accession</span>
              <input
                type="text"
                value={accession}
                onChange={(e) => setAccession(e.target.value)}
                placeholder="例如 NC_003888.3 或 GCA_000156475.1"
                className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2.5 font-mono text-sm dark:border-slate-700 dark:bg-slate-900"
                required
              />
              ...
              return (
                <button
                  ...
                  className={`rounded-lg border p-3 text-left transition-colors ${
                    active ? "border-indigo-500 bg-indigo-50/60 dark:border-indigo-400 dark:bg-indigo-950/30"
        ...
        >
          ...
          <div className="grid gap-4 border-t border-slate-200 p-4 sm:grid-cols-2 dark:border-slate-800">
            ...
            <label className="block">
              ...
              <input type="number" min={100} max={1_000_000} step={100}
