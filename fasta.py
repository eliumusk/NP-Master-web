...
# Color per BGC type, used in BED itemRgb so IGV.js renders different classes
# in different colors. Hex tuned to be readable on light + dark themes.
BGC_TYPE_COLORS: dict[str, str] = {
    "Alkaloid":   "220,38,38",    # red
    "Terpene":    "22,163,74",    # green
    "NRP":        "37,99,235",    # blue
    "Polyketide": "124,58,237",   # purple
    "RiPP":       "234,88,12",    # orange
    "Saccharide": "219,39,119",   # pink
    "Other":      "100,116,139",  # slate
}
_DEFAULT_COLOR = "124,58,237"  # fallback purple


def csv_regions_to_bed(csv_path: Path, bed_path: Path, name_prefix: str = "BGC") -> int:
    """Convert decoder CSV to a BED9 with per-type colors (itemRgb).

    Reads columns: contig, start, end, score, v4_1_type (optional).
    Writes BED9: contig start end name score strand thickStart thickEnd itemRgb.
    ...
    with open(csv_path, newline="") as fin, open(bed_path, "w") as fout:
        ...
        for i, row in enumerate(rd, start=1):
            ...
            score_int = max(0, min(1000, int(round(score * 1000))))
            bgc_type = (row.get("v4_1_type") or "").strip() or "Other"
            color = BGC_TYPE_COLORS.get(bgc_type, _DEFAULT_COLOR)
            name = f"{name_prefix}_{i:04d}_{bgc_type}"
            start = int(row["start"])
            end = int(row["end"])
            fout.write(
                f"{row['contig']}\t{start}\t{end}\t{name}\t{score_int}\t.\t{start}\t{end}\t{color}\n"
            )
            n += 1
    return n
